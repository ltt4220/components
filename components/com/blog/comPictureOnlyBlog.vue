<template>
  <div :id="id" :uid="uid" class="zm-component-main" :style="{position: 'absolute', width: style.width+'px',height: style.height+'px',left: style.left+'px',top: style.top+'px',zIndex: style.zIndex,boxShadow:settingData.pictureShadow.num}" @mousedown.stop.left="other($event),mousedown($event)" @contextmenu.stop.prevent="ther($event),openMenu($event)">
    <div class="zm-component-blog-content onlyPic" data-type="words" :data-num="settingData.blogNum.value">
      <span v-for="(item,i) in setList" :key="i">
        <div class="zm-component-blog-box" :data-sortid="item.fId" :data-show="true" :data-time="item.fCreateTime" :data-title="item.fTitle" :style="{backgroundColor:settingData.BoxBackgroundColor.value,paddingLeft:settingData.paddingLeftRight.value,paddingRight:settingData.paddingLeftRight.value}">
          <div class="zm-component-blog-mediaBox medialBoxOne" :style="{padding:settingData.pictureLRPadding.value,width:settingData.pictureWidth.value,height:settingData.pictureHeight.value}">
            <a class="zm-component-blog-bgImg" :data-show="item.ahref" :style="{display:item.imgDis,backgroundImage:'url('+item.bgUrl+')'}">
            </a>
            <a class="zm-component-blog-bgVideo" :style="{display:item.videoDis}">
          ...
          <div class="zm-component-blog-detail">
            <div class="zm-component-blog-upcont">
              ...
              <div class="zm-component-blog-mediaBox mediaBoxTwo">
                <div class="zm-component-blog-bgImg" :style="{display:item.imgDis,backgroundImage:'url('+item.bgUrl+')'}">
...
<script>
import callBack from "./onlyBlog/callBackFunction.js";
  ...
  data() {
    return {
      ...
      settingData: {
        ...
        pictureShadow: {
          value: "0px 0px 5px rgba(0,0,0,0.2)",
          trueVal: "0px 0px 5px rgba(0,0,0,0.2)",
          isOpen: false,
          id: "blog1-6",
          valName: "pictureShadow"
        },
